...
class CI_JqueryUtils extends CI_JsUtils {
...
//------------------------------------------------------------------------
    ...
    public static function _postForm($url,$form,$responseElement,$validation=false,$function=NULL,$attr="id",$immediatly=false){
        $function=isset($function)?$function:"";
        $retour="url='".$url."';\n";
        if($attr=="value")
            $retour.="url=url+'/'+$(this).val();\n";
    ...
    public function _doSomethingOn($element,$someThing,$param="",$function=""){
        $script= "$('".$element."').".$someThing."(".$param.");\n".$function;
        $this->jquery_code_for_compile[] = $script;
        return $script;
    }
}
